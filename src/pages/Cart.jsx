    ...
    const totalPrice = () => {
        try {
            ...
            cart?.forEach((item) => {
                ...
                const cleaned = item.price.toString().replace(/[^\d.]/g, '');
                                                ...
                                                <div className='card-body'>
                                                    <div className="text-center">
                                                        ...
                                                        {auth?.user?.address ? (
                                                            <>
        <div className="mt-2">
  {/* Debug messages to help you understand what's missing */}
  {!clientToken && <p className="text-danger">🔒 Braintree token not loaded.</p>}
  {!auth?.token && <p className="text-warning">🔑 Please login to proceed.</p>}
  {!cart?.length && <p className="text-info">🛒 Cart is empty.</p>}
  {!auth?.user?.address && <p className="text-danger">📍 Address is missing!</p>}

  {/* Only show DropIn if everything is present */}
