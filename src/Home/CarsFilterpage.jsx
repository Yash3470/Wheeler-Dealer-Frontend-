...
const CarsHome = () => {
                ...
                <div className="row" style={{ marginBottom: '100px', marginTop: '-50px' }}>
                    ...
                    <div className="col-md-12 col-lg-9">
                        {loading ?
                            ...
                            <div className="row">
                            {cars
    .filter((car) => car && (search.toString().toLowerCase() === '' ? car : car.name.toLowerCase().includes(search)))
    .filter((car) => car && (selectedBrands.length === 0 || selectedBrands.includes(car.brand?._id)))
    .filter((car) => {
        if (!car) return false;
    ...
    .map((p) => (
        console.log(process.env.REACT_APP_API_URL+p.productPictures),
        p && (
                ...
                <div className="card">
                    <div className="d-flex justify-content-between p-3">
                        ...
                        >
                            ...
                            >
                                <img
                                    src={process.env.REACT_APP_API_URL+p.brand?.brandPictures || '/default-image.jpg'}
                                    alt={p.brand?.name || 'Unknown Brand'}
                    ...
                    <Link to={`/car/${p.slug || '#'}`} className='text-center'>
                        <img
                            src={process.env.REACT_APP_API_URL+p.productPictures?.[4] || '/default-image.jpg'}
                            alt={p.name || 'Unknown Car'}
                            style={{ maxWidth: '100%', maxHeight: '130px', objectFit: 'contain' }}
                            className='border rounded'
                        />
                    </Link>
                    <div className="card-body">
                        <h4 className="text-center mb-4 respName">{p.name || 'Unknown Car'}</h4>
                        <div className="d-flex justify-content-between">
                            <h6 className='respBrand'><PiCurrencyInrFill /> : {p.price || 'N/A'} Lakhs</h6>
                            <h6 className='respBrand'><BsFuelPumpFill /> : {p.fuelType || 'N/A'}</h6>
                        </div>
                        <div className="d-flex justify-content-between my-2">
                            <h6 className='respBrand'><TbStars /> : {p.safetyrating || 'N/A'}</h6>
                            <h6 className='respBrand'><MdAirlineSeatReclineExtra /> : {p.seater || 'N/A'} Seater</h6>
                        </div>
